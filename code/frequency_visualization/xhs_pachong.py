...
# # 读取CSV文件
# file_path1 = 'data/xhs/xhs_pachong.csv'
...
# # 将词频结果保存到CSV文件
# # 将词频结果保存到TXT文件
# with open('data/xhs/pachong_word_frequency.txt', 'w', encoding='utf-8') as file:
...
# 读取文件并解析数据
word_frequencies = {}
with open('data/xhs/pachong_word_frequency.txt', 'r', encoding='utf-8') as file:
    for line in file:
        # 每行格式是 "词汇:频率"
        line = line.strip()
        if line:  # 确保行不为空
...
wordcloud.to_file('data/xhs/xhs_pachong.png')
